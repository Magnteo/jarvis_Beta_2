    ...
    if os.path.exists(file):
        with open(file, "r", encoding="utf-8") as files:
            ...
            for read in lines:
                ...
                if len(parts) == 3:
                    cain = {"time": parts[0], "priority": parts[1], "text": parts[2]}
                    tasks.append(cain)
...
def task_menu():
    ...
    while True:
        ...
        if command == "вийти":
            ...
            print(
                edit_task(old_text, new_text, new_time, new_priority, tasks, file_txt)
            )
        elif command == "видалити":
            text_to_delete = input(
                "Введіть текст задачі, яку хочете видалити: "
            ).strip()
            print(delet_task(text_to_delete, tasks, file_txt))
        elif command == "очистити":
            confirm = (
                input("Ви впевнені, що хочете видалити всі задачі? (так/ні): ")
                .strip()
                .lower()
            )
            if confirm == "так":
            ...
            print(
                "Команда не розпізнана. Спробуйте: додати, редагувати, видалити, очистити, показати, вийти."
            )
